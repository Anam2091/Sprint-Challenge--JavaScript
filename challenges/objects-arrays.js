...
const universities = [];
const newgradarray = graduates.map(x=> x.university)
console.log(newgradarray);
...
const contactInfo = [];
const cont = graduates.map(x=>x.first_name + ' ' + x.email)
console.log(cont);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = [];
const newuni = graduates.filter(x=>{
 return x.universities.includes('Uni')
...
const lower = zooAnimals.map(x=>x.animal_name.toLowerCase())
...
const result = zooAnimals.filter(element => element.population>5)
console.log(result);
...
const result = zooAnimals.map(element => element.population);
const reducer = (accumulator, currentValue) => accumulator + currentValue;
const mappy = result.reduce(reducer)
console.log(mappy);
